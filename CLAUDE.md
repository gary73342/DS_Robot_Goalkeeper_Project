...
- **定位層**：EKF 主導（`EKF_Localization.cpp`），`KidnapRecovery.cpp` 負責綁架恢復（R-MCL 簡化版假設管理）
...
大架構（感知、定位、攔截）已完成。詳細待辦事項見 `TODO.txt`，重點摘要：

**待解決**：
1. 只偵測到 1 根門柱時的 EKF 處理方式（有三個方案 B1/B2/B3）
2. 攔截模式下移動過慢（進入攔截模式後改全速）
3. 攔截完球後的行為邏輯（有三個方案 A1/A2/A3）

**待優化**：
- 綁架恢復後 rviz 定位顯示飄移問題
- 降低相機解析度提升 YOLO 推論速度（需重新校準 Homography）

**待測試**：
- KIDNAP_RECOVERY 完整流程（觸發 → 旋轉 → 假設收斂 → 切回 EKF）
- `/hypothesis_cloud` RViz 假設點雲顯示（RECOVERY 期間）
- 終端機輸出格式確認
